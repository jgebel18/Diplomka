...
class MakeIterationProcessforGamma:
  ...
  #Here is a construction of class variables
    ...
    def GiveSolutionofNonlinear(self, Gamma, ):
        ...
        analytic_roots=self.ZeroOmegaCase(Gamma)
        analytic_root= 0.0
        # najdi index nejbližší analytickému řešení
        mid_index = np.argmin(np.abs(self.omegavalues - analytic_root))
        complete_solution[mid_index] = analytic_root

        # --- pokračuj doprava ---
        omega_0 = analytic_root
        for i in range(mid_index + 1, len(self.omegavalues)):
            omega = self.omegavalues[i]
            sol = root_scalar(
                f=self.QuarticEquation,
                method='newton',
                args=(Gamma, omega),
                fprime=self.QuarticEquationDerivative,
                x0=omega_0,
                maxiter=1000
            )
            complete_solution[i] = sol.root
            omega_0 = sol.root
    ...
    def GenerateSolution(self, Gamma):
        ...
        self.PF.PlotRootOfEquation(values, self.omegavalues)
